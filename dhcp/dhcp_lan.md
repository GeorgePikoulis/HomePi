...
Connection: `LAN-10.0.69` bound to `eth1` (USB NIC).  
Autoconnect & stability tweaks:

```bash
nmcli connection modify LAN-10.0.69 connection.autoconnect yes
nmcli connection modify LAN-10.0.69 connection.autoconnect-priority 10
nmcli connection modify LAN-10.0.69 connection.wait-device-timeout 30
```
...
```
/etc/NetworkManager/system-connections/LAN-10.0.69.nmconnection
```
